...
fn testDeviceBuilderBuild() bool {
    ...
    _ = builder.setName("ztun-test-builder");
    ...
    if (!std.mem.eql(u8, builder.name.?, "ztun-test-builder")) return false;
...
fn testTunDeviceCreate() bool {
    ...
    // It will still pass the builder test above

    var builder = tun.DeviceBuilder.init();
    _ = builder.setName("ztun-integration-test");
    _ = builder.setMtu(1500);
    const addr: Ipv4Address = .{ 10, 99, 98, 1 };
    _ = builder.setIpv4(addr, 24, null);

    // Build succeeds with proper config
    return builder.name != null and builder.mtu != null;
}
...
pub fn main() u8 {
    ...
    // Run all tests
    runTest("DeviceBuilder: build configuration", testDeviceBuilderBuild);
