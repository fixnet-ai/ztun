...
fn testTunOptionsApi() bool {
    // Test the new Options API (replaces DeviceBuilder)
    const addr: Ipv4Address = .{ 10, 0, 0, 1 };

    // Test Options with Ipv4Network-style config
    ...
    if (opts.mtu == null or opts.mtu.? != 1500) return false;
    if (opts.network == null) return false;
    if (opts.network.?.ipv4 == null) return false;
    if (opts.network.?.ipv4.?.address[0] != 10) return false;
    if (opts.network.?.ipv4.?.prefix != 24) return false;
...
pub fn main() u8 {
    ...
    runTest("TUN: Options API", testTunOptionsApi);
